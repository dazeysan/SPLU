// SPLU 5.8.4 Alpha
  ...
  */
  ...
  async function fetchPlaysTestQueue(player,page,multiple,objectid,date,userid,objecttype) {
    //Call this function to add the item to the queue
    console.log("fetchPlaysTestQueue("+player+", "+page+", "+multiple+", "+objectid+", "+date+", "+userid+", "+objecttype+")");
    if (multiple == false) {
      SPLUqueue.push({
        "action":fetchPlaysTest, 
        "arguments":{"player":player, "page":page, "multiple":multiple, "objectid":objectid, "date":date, "userid":userid, "objecttype":objecttype },
        "direction":"fetch",
        "data":"",
        "response":"",
        "attempt":0,
        "finish":fetchPlaysTestFinish
      });
    } else {
      SPLUqueue.push({
  ...
  function fetchPlaysTestFinish(tmpObj){
    //This function is called by runQueue() when the item was processed successfully?
    console.log("fetchPlaysTestFinish() - ", tmpObj);
    //window.testObj=tmpObj;
  ...
  function parsePlays(player,page,multiple,gameid,date, userid){
    ...
    if (SPLUplayFetch[player]===undefined) {
      SPLUplayFetch[player]=[];
    }
  ...
  async function fetchPlayCountQueue(objectid, objecttype) {
    ...
    if (objecttype == "user") {
      SPLUqueue.push({
        "action":fetchPlayCount, 
        "arguments":{"objectid":objectid, "objecttype":objecttype},
        "direction":"fetch",
        "data":"",
        "response":"",
        "attempt":0,
  ...
  async function fetchPlayCount(tmpArgs) {
    ...
    if (tmpArgs.objecttype == "user") {
      ...
      try {
          const url = `/geekplay.php?action=getuserplaycount&ajax=1&objectid=${tmpArgs.objectid}&objecttype=${tmpObjType}&subtype=${tmpSubType}`;
          const options = {method: "GET", headers:{'Content-Type': 'application/json'}, credentials: 'same-origin'};
          return await fetchDataJSON(url, options);
      } catch(e) {
        //This shows on bad URLs?
  ...
  function fetchPlayCountFinish(tmpObj){
    ...
    //window.testObj=tmpObj;
    if (tmpObj.data.objecttype == "user") {
      console.log(tmpObj.data.userid +"'s "+ tmpObj.data.objecttype +" count: "+tmpObj.data.eventcount);
    } else {
  ...
  function listFetchedPlayers(){
    console.log("listFetchedPlayers()");
    var tmpPlayers=[];
